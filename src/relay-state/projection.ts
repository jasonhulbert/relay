// Field-projection chokepoint: orchestrator-visible narrative is NEVER admissible
// to the critic. The executor's self-report is INADMISSIBLE as evidence of done: a
// confident narrative sets the
...
// A per-call usage observation a critic emits for per-call usage attribution. It is
// pure instrumentation — write-only telemetry the orchestrator persists keyed by the
// graded node — and carries NOTHING into the critic, so it cannot reopen the
// narrative leak (the projection is still spec + diff + evidence only). Kept
...
// The one constructor. Copies only the critic-admissible fields off the
...
// projection. It is deliberately SEPARATE from the `CriticView`: nothing here is
// graded as evidence, so none of it can launder the narrative back in (the
// evidence-only-critic property test still pins the view to spec + diff +
...
// code-owned MCP loop that populates it is not built yet (today it is empty).
export interface CriticContext {
  ...
  // Optional usage sink for per-call cost attribution: the orchestrator supplies it
  // at the call site (where it knows the graded node), and the critic emits its
  // model call's usage into it for node-attributed persistence. Absent on the stub
  // path and in direct unit calls.
  readonly onUsage?: (usage: CriticCallUsage) => void;
}

// The independent critic itself (a separate agent, different provider by
// default). Provided by the spine; here we only fix its signature. It is
