// Footprint primitives for the concurrency law. A child's footprint is its predicted
// resource use — currently the repo-relative write globs it will touch
// (`Footprint.writeGlobs`). Two structural facts come out of a footprint:
//
//   - DISJOINTNESS schedules concurrency: a layer's siblings may run in parallel only
//     if their footprints are provably disjoint. `footprintsDisjoint` is the
//     predicate; it answers from the declared globs alone (Rule 5 — code decides, not
//     a model).
//   - The footprint is a HINT, not a sandbox: it schedules, it does not guarantee
//     correctness. A child whose ACTUAL writes escape its declared footprint is a
//     *loud* violation — `footprintEscapes` finds the offending paths and the
//     orchestrator throws `FootprintViolation`, which the escalation ladder absorbs
//     as a failed attempt (the loud-violation catch; the silent one is the
//     integration gate).
//
...
// side. The bias on the unknown is serial: a footprint that cannot be proven
...
// The shared tier-A session resource: the single logged-in headed session a visual
// leaf drives. A leaf that contends on it names it in its footprint's `resources`, so
// two such leaves are not disjoint and the scheduler serializes them — the visual-kind
// specialization of "shared resource ⇒ serial".
export const TIER_A_SESSION = 'tier-a-session';

// Are two footprints disjoint — can no concrete repo path be written by both
// children AND do they contend on no common named resource (concurrency-law
// condition 1)? A footprint that writes nothing (no globs) and names no resource is
// disjoint from everything. Otherwise disjoint iff no write-glob pair can intersect
// AND the two resource sets share no member — a shared resource (e.g. the tier-A
// session) is exactly what the concurrency law forbids running concurrently, even
// with non-colliding writes.
...
// The actual write paths that escape a declared footprint: a repo-relative
...
// A loud footprint violation: a child touched a resource outside its declared
...
export class FootprintViolation extends Error {
  ...
  report(): string {
    return `Loud footprint violation: ${this.message}. Absorbed by the escalation ladder as a failed attempt.`;
  }
}
