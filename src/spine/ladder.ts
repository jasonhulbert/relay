// The escalation ladder (design §3.7, §3.9): the bounded sequence a leaf walks
// when its critic returns FAIL, before terminal `blocked`. Rungs, in order:
// retry (same provider) → swap-provider → raise-tier → promote (leaf→branch,
// re-decompose). A too-big judgment jumps straight to promote (design §3.9:
// "judged too big → PROMOTE leaf→branch").
//
...
// dispatch and persistence (C2). The two things that consume its output land in
// later M3 phases: the `promote` rung's atomic leaf→branch transaction is
// Phase 2, and the `blocked` record + halt-and-surface that consume `exhausted`
// are Phase 3.
...
// to promote. In M3 these are injected by controllable stubs; M4 sources them
// from real providers.
export type AttemptSignal = 'pass' | 'fail' | 'too-big';

// Why the ladder ended without success.
export type ExhaustionReason =
  // A budget cap halted the ladder mid-walk (the rail guarantee, §3.7).
...
export type LadderStep =
  ...
  // (Phase 3) and runs the unified failure rule.
...
export class EscalationLadder {
  private nextIndex = 0;
  // The rungs actually handed out, in order — the audit trail Phase 3's blocked
  // record reports as "rungs spent" (design §3.7).
  private readonly spent: Rung[] = [];

  constructor(private readonly caps: RailCaps) {}

  // Rungs handed out so far, in order. Read by the (Phase 3) blocked record.
  ...
  // would be next — the rail guarantee comes before the rung walk (design §3.7).
