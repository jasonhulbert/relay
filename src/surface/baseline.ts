// The baseline pipeline — the baseline-diff rung of the match-granularity ladder,
// deliberately deferred until the structural rung landed. It captures, promotes,
// versions, and budgets baselines safely, and plugs into the visual critic as the
// injected `BaselineGrader`.
//
// Four design pins live here:
//   - structural-or-better promotion: a baseline is captured from a passing run, never
...
//   - content-addressed store split: the binary lives in a durable, content-addressed
//     store that is a SIBLING of `.relay/` (so the evidence compactor, which scans only
//     `.relay/evidence/<runId>/`, can never reach it); `.relay/` holds only the
//     Markdown ref — hash, outcome-id, granularity, version, tolerance.
//   - re-versioning: replacing a known-good baseline is the one spot a model
//     cannot rule (intended redesign vs regression is exactly the judgment the
//     baseline is not an oracle for), so code NEVER silently overwrites: a
...
//   - flake budget: two per-outcome fields — a spatial tolerance (perceptual-diff
//     distance at-or-below which a diff passes) and the temporal retry count from
//     the drift-re-dispatch transient path. A persistent above-tolerance diff against
//     a HEALTHY app surfaces a mismatch decision — never an auto-pass and never a
//     silent fail.
//
...
// once the pipeline is plumbed into the orchestrator; tests script distances) and the
// `MismatchSink` (which routes to the human decision inbox / web view; tests record
// it). The module is not yet wired into the orchestrator loop.
...
// The baseline-diff arm of the verification union — the only granularity
// this pipeline grades.
type BaselineVerification = Extract<VisualVerification, { granularity: 'baseline-diff' }>;

// The durable Markdown ref `.relay/baselines/<outcomeId>.md` carries. The
...
export interface BaselineRef {
  ...
  // intent; promotion is gated at structural-or-better).
...
// decoded PNGs) is wired later; `exactBytesDiffer` is the honest current default and
...
// The current default differ: identical bytes → 0, anything else → 1. A faithful lower
// bound (it never reports a within-tolerance pass for a frame that actually changed)
// that keeps the pipeline runnable before the perceptual differ is wired in — the
...
// Why a known-good baseline is being asked to change. `re-version` is an
...
// It mirrors a human-decision shape WITHOUT coupling to relay-state's
// human-owned `cancel` inbox — the orchestrator maps it onto the surfacing region
// exactly as the spine maps a `VisualGrade` onto a durable `CriticVerdict`.
...
// Where a surfaced mismatch goes. Injected so the pipeline never writes the
// human-owned inbox itself; the orchestrator routes it to the decision inbox / web
// view, tests record it.
export type MismatchSink = (mismatch: BaselineMismatch) => Promise<void>;

// The flake budget: the two per-outcome knobs. Conservative defaults — strict
...
export interface FlakeBudget {
  ...
  tolerance: number;
  // Re-capture attempts for a transient above-tolerance diff (the temporal path).
  retries: number;
}

export const DEFAULT_FLAKE_BUDGET: FlakeBudget = { tolerance: 0, retries: 2 };

// Everything the pipeline needs that the orchestrator owns. `relayDir` is where
...
// The durable, content-addressed baseline binary store. A SIBLING of `.relay/`
...
// Capture-and-promote the FIRST baseline for an outcome. Two gates are
// code-enforced rather than trusted to the caller:
//   - structural-or-better — an `intent` granularity is refused (intent is the
...
//     the caller must go through `requestReVersion` + human approval.
...
export async function promoteBaseline(
  ...
  if (opts.granularity === 'intent') {
    throw new Error(
      'baseline promotion is gated at structural-or-better; refusing to promote an intent-only pass',
    );
  }
  const existing = await readBaselineRef(ctx.relayDir, ctx.outcomeId);
  if (existing) {
    throw new Error(
      `baseline for "${ctx.outcomeId}" already exists (v${existing.version.toString()}); replacing a known-good baseline requires requestReVersion + human approval`,
    );
  }
...
// A re-version ATTEMPT on a known-good baseline. It NEVER overwrites: it stores
...
// Apply a human-APPROVED re-version — the deterministic transform the approval
// decision triggers (the orchestrator drains the approval and calls this). Bumps the
// version,
...
// auto-promote; `within-tolerance` is a pass; `above-tolerance` may be a
...
// Diff one capture against the stored baseline. No baseline yet → auto-promote
...
// Probe the app's liveness the same way drift re-dispatch does (a trivial
// `query_state`): a mismatch is only surfaced against a HEALTHY app. If the probe
// throws, the app died —
...
// Grade a baseline-diff verification end to end (the baseline-diff rung +
// structural-or-better promotion + the flake budget). Re-captures up to `retries`
// times to absorb a transient above-tolerance
...
// Captures are scoped to the verification's named element's semantic subtree when one
// is declared.
export async function verifyBaselineDiff(
  ...
  // app; a dead app is a real failure on its own terms, not a redesign judgment.
...
// rung. Closes over the pipeline context so `replayAndGrade` stays
